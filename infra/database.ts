...
export async function getNewClient(): Promise<Client> {
  ...
  // injects, and our local .env.development also sets it for dev. Fall back
  // to the individual POSTGRES_* vars so tooling pointing at the local Docker
  // Postgres still works without a URL.
  //
  // SSL: Neon refuses cleartext connections even when the URL omits
  // `sslmode=require`. Force ssl on in production regardless of which branch
  // we hit.
