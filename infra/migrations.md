...
In production and preview deploys (Vercel), migrations run during the build step via the `vercel-build` script (`node-pg-migrate up` against `DATABASE_URL_NON_POOLING`, then `next build`). Each Vercel preview gets its own Neon DB branch through the Neon-Vercel integration, so migrations are applied independently per branch.
